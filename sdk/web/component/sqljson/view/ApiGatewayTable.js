var ApiGatewayTable = function ApiGatewayTable(cfg)
{
    let frm = {};
    let btnAdd;
    let btnRefresh;
        ...
        frm = new PanelTable(
            {
                ...
                EnableSearch: true,
                Schema: new ApiGatewaySchema(),
                OnItemRowClick: function (e, tr, td, item, schitem)
                {
                    let frm = new ApiGatewayForm({
                        Id: item.EndPoint, Label: item.Name, ParentHtmlId: "modal", Theme: cfg.Theme, 
    ...
    function getData(grid)
    {
        ...
        _adminCtrl.GetApiGateways(function (data)
        {
            if (data == null)
            {
